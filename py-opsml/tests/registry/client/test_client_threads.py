...
def test_crud_artifactcard_threads(
    ...
    with OpsmlTestServer():
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            # Submit all tasks
            data_future = executor.submit(crud_datacard, pandas_data)
            datacard, data_registry = data_future.result()

            # Model card depends on datacard, so we need to wait
            model_future = executor.submit(
                crud_modelcard, random_forest_classifier, datacard
            )
            prompt_future = executor.submit(crud_promptcard, chat_prompt)

            # Get results
            modelcard, model_registry = model_future.result()
            promptcard, prompt_registry = prompt_future.result()

            # Delete cards
            delete_futures = [
                executor.submit(delete_card, card, registry)
                for card, registry in [
                    (promptcard, prompt_registry),
                    (modelcard, model_registry),
                    (datacard, data_registry),
                ]
            ]
