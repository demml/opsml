    ...
    def save_data(self, path: Path, **kwargs) -> Path:
        ...
        Args:
            path (Path) :
                Base path to save the data to
            **kwargs:
                Additional keyword arguments to pass to the save (see the following kwargs section).
    ...
    def load_data(self, path: Path, **kwargs) -> None:
        ...
        Args:
            path (Path):
                Base path to load the data from
            **kwargs:
                Additional keyword arguments to pass in (see the following kwargs section).
    ...
    def save_data(self, path: Path, **kwargs) -> Path:
        ...
        Kwargs:
            engine ({'auto', 'pyarrow', 'fastparquet'}):
                Parquet library to use. If 'auto', then the option io.parquet.engine is used. The default io.parquet.engine
                behavior is to try 'pyarrow',
            ...
            index (bool | None):
                ...
                However, instead of being saved as values, the RangeIndex will be stored as a range in the metadata
                so it doesn't require much space and is faster.
    ...
    def load_data(self, path: Path, **kwargs) -> None:
        ...
        Kwargs:
            engine ({'auto', 'pyarrow', 'fastparquet'}):
                ...
                The default io.parquet.engine behavior is to try 'pyarrow', falling back to 'fastparquet' if 'pyarrow' is
                unavailable. Default is 'auto'.
            columns (list | None):
            ...
            use_nullable_dtypes (bool):
                ...
                (only applicable for the pyarrow engine) As new dtypes are added that support pd.NA in the future, the output
                with this option will change to use those dtypes. Note: this is an experimental option, and behaviour
                (e.g. additional support dtypes) may change without notice. Default is False. Deprecated since version 2.0.
            ...
            filters (list[tuple] | list[list[tuple]] | None):
                ...
                meaning that no OR operation between set of filters is to be conducted. Using this argument will NOT result in
                row-wise filtering of the final partitions unless engine="pyarrow" is also specified. For other engines,
                filtering is only performed at the partition level, that is, to prevent the loading of some row-groups
                and/or files. Default is None.
            **kwargs:
                Any additional kwargs are passed to the engine.
...
class TorchData(DataInterface):
    ...
    def load_data(self, path: Path, **kwargs) -> None:
        ...
        Kwargs:
            ...
            weights_only:
                Indicates whether unpickler should be restricted to loading only tensors, primitive types, dictionaries and
                any types added via torch.serialization.add_safe_globals().
            mmap:
