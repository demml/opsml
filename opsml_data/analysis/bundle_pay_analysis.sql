-- Pay analysis for bundles predictions made model trained on bundle data
-- UNCOMMENT FOR TESTING
--WITH PREDS AS (
--
--SELECT
--  TIME_BUNDLE_ID AS BUNDLE_ID,
...
INNER JOIN (SELECT * FROM DATA_SCIENCE.OPSML_ORDER_BUNDLE_EVAL_DATA WHERE BUNDLE = 1) AS C
...
-- BUNDLE EMS (NEW)
IFNULL(B.PICK_NEW_EM, 1)                                                        AS PICK_BEM_NEW,
IFNULL(C.CHECKOUT_NEW_EM, 1)                                                    AS CHECKOUT_BEM_NEW,
IFNULL(D.DELIVERY_NEW_EM, 1)                                                    AS DELIVERY_BEM_NEW,
CASE WHEN BUNDLE_TYPE ='TLMD' THEN 1 ELSE IFNULL(E.DROP_OFF_NEW_EM, 1) END      AS DROP_OFF_BEM_NEW,
CASE WHEN BUNDLE_TYPE ='TLMD' THEN 1 ELSE IFNULL(F.WAIT_NEW_EM, 1) END          AS WAIT_BEM_NEW,
        

  ---- NEW PREDICTIONS FOR EACH FULFILLMENT LEG
CASE 
  WHEN PRED_WAIT_TIME = FP_WAIT_TIME THEN FP_WAIT_TIME
  ELSE ((PRED_WAIT_TIME * WAIT_BEM_NEW))  END                                   AS PRED_WAIT_TIME_ADJ, 
PRED_WAIT_TIME_ADJ/60 * GOAL_HOURLY_WAGE                                        AS ESTIMATED_WAIT_PAY_NEW,
  
CASE 
  WHEN PRED_DROP_TIME = FP_DROP_TIME THEN FP_DROP_TIME
  ELSE ((PRED_DROP_TIME * DROP_OFF_BEM_NEW))  END                               AS PRED_DROP_OFF_TIME_ADJ, 
...
CASE 
  ...
  ELSE ((PRED_PICK_TIME * PICK_BEM_NEW))   END                                  AS PRED_PICK_TIME_ADJ, 
...
CASE
  ...
  ELSE ((PRED_CHECKOUT_TIME * CHECKOUT_BEM_NEW)) END                            AS PRED_CHECKOUT_TIME_ADJ,
...
CASE
  ...
  ELSE ((PRED_DELIVERY_TIME * DELIVERY_BEM_NEW)) END                            AS PRED_DELIVERY_TIME_ADJ,
PRED_DELIVERY_TIME_ADJ/60 * GOAL_HOURLY_WAGE                                    AS ESTIMATED_DELIVERY_PAY_NEW ,

-- TEMP LOGIC UNTIL WE HAVE DRIVE TIME
CASE 
    WHEN BUNDLE_TYPE IN ('TLMD', 'TARP') THEN
      FP_OVERALL_TIME - FP_WAIT_TIME - FP_DROP_TIME + PRED_WAIT_TIME_ADJ + PRED_DROP_OFF_TIME_ADJ
...
SELECT 
  ...
  --METRO,
  --A.SHOPPER_EXPERIENCE,
  --COUNT(DISTINCT DELIVERY_DATE::DATE) AS DAYS,
  --CASE WHEN NBR_ORDERS >1 THEN 1 ELSE 0 END AS BUNDLE,
  SUM(NBR_ORDERS) AS NBR_ORDERS_WORKED,
  COUNT(DISTINCT BUNDLE_ID) AS NBR_BUNDLES,
  ...
  -- PAY PER BUNDLE
    ESTIMATED_PAY_OLD/NBR_BUNDLES                      AS PPB_OLD,
  ESTIMATED_PAY_NEW/NBR_BUNDLES                        AS PPB_NEW,
  PPB_NEW - PPB_OLD                                    AS PPB_DIFF,


  -- NEW PAY THAT IS WITJIN 25 CENTS OF HOURLY WAGE
  TO_NUMBER((COUNT_IF(ABS(DIV0(ESTIMATED_OVERALL_PAY_FLOOR_NEW, (ACTUAL_OVERALL_TIME/60)) - GOAL_HOURLY_WAGE) < 0.25)/NBR_BUNDLES)*100,10,2) AS PCT_BUNDLES_WITH_PPH_NEW_MINUS_GOAL_WITHIN_25CENTS,

  -- BUNDLES THAT WERE FLOORED
  TO_NUMBER(COUNT_IF(ESTIMATED_OVERALL_PAY_FLOOR_OLD <= PER_ORDER_FLOOR) / NBR_BUNDLES * 100,10,2) as PCT_LIVE_BUNDLE_WITH_BASE_PAY_FLOORED, 
  TO_NUMBER(COUNT_IF(ESTIMATED_OVERALL_PAY_FLOOR_NEW <= PER_ORDER_FLOOR) / NBR_BUNDLES * 100,10,2) as PCT_NEW_BUNDLES_WITH_BASE_PAY_FLOORED,
  ...
  --bems
  TO_NUMBER(MEDIAN(PICK_BEM_NEW),10,2)              AS PICK_EM,
  TO_NUMBER(MEDIAN(CHECKOUT_BEM_NEW),10,2)          AS CHECKOUT_EM,
  TO_NUMBER(MEDIAN(DELIVERY_BEM_NEW),10,2)          AS DELIVERY_EM,
  TO_NUMBER(MEDIAN(WAIT_BEM_NEW),10,2)              AS WAIT_EM,
  TO_NUMBER(MEDIAN(DROP_OFF_BEM_NEW),10,2)          AS DROP_EM
