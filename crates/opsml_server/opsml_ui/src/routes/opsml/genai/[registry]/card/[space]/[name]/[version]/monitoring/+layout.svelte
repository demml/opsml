<script lang="ts">
  ...
  let currentDriftType = $derived($page.params.drift as DriftType);
  let currentConfig = $derived(driftTypeConfig[currentDriftType] || driftTypeConfig[DriftType.Custom]);
  let basePath = $derived(`/opsml/genai/${$page.params.registry}/card/${data.metadata.space}/${data.metadata.name}/${data.metadata.version}/monitoring`);
  let monitoringData = $derived($page.data.monitoringData);

  onMount(() => {
    if (monitoringData?.status === 'success' && monitoringData.selectedTimeRange) {
      timeRangeState.updateTimeRange(monitoringData.selectedTimeRange);
    }
  });
