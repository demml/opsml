...
<div class="flex-1 mx-auto w-9/12 pb-10 flex justify-center overflow-auto px-4">
  <div class="grid grid-cols-1 lg:grid-cols-6 gap-4 w-full pt-4 items-start">
    ...
    {#if data.readme.exists}
      <div class="col-span-1 md:col-span-4 gap-1 flex flex-col rounded-base border-black border-3 shadow bg-surface-50">
        <CardReadMe
          name={metadata.name}
    ...
    {:else}
      <div class="col-span-1 lg:col-span-4 mx-auto gap-1 flex flex-col rounded-base border-black border-3 shadow bg-primary-100 w-1/2 h-1/4">
