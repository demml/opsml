<script lang="ts">
  ...
  let ThemeIcon = $derived(themeIcons[themeStore.mode]);
...
<nav class="fixed left-0 top-0 z-[60] mx-auto flex w-full items-center border-b-4 border-border bg-primary-700 border-b-2 border-black px-5 h-14">
  <div class="mx-auto flex w-full items-center justify-between px-8 lg:px-10">
    ...
    <div class="flex items-center justify-end gap-4 m800:w-[unset] m400:gap-2">
      ...
      <button
        aria-label={themeLabels[themeStore.mode]}
        title={themeLabels[themeStore.mode]}
        onclick={() => themeStore.toggle()}
        class="m800:hidden flex gap-2 items-center justify-center rounded-base border-2 border-black shadow p-2 shadow-hover bg-surface-50 w-9 h-9 text-black"
      >
        <ThemeIcon size={18} />
      </button>
      ...
      <a aria-label="github" href="https://github.com/demml/opsml" class="m800:hidden flex gap-2 items-center justify-center rounded-base border-2 border-black shadow p-2 shadow-hover bg-surface-50 w-9 h-9 text-black">
        ...
        <button aria-label="login" onclick={logInHandle} class="m800:hidden flex gap-2 items-center justify-center rounded-base border-2 border-black shadow p-2 shadow-hover bg-surface-50 w-9 h-9 text-black">
          <KeySquare size={18} class="text-primary-600"/>
        </button>

        <div
          aria-label="user"
          class="m800:hidden relative flex items-center justify-center rounded-base border-2 border-black shadow p-2 shadow-hover bg-surface-50 w-9 h-9 text-black"
        >
          <UserDropdown/>
        </div>
      </div>
