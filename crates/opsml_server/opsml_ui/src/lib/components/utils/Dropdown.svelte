...
<div class="dropdown relative {width}">
    <button 
        ...
        class="w-full text-sm px-2 {py} bg-primary-500 text-black border-black border-2 rounded-base flex justify-between items-center"
    >
    ...
    {#if isOpen}
      <div class="absolute top-full left-0 right-0 mt-1 bg-primary-500 border-black border-2 rounded-base z-[100]">
        <div class="max-h-72 overflow-y-auto px-1 py-1 custom-scrollbar">
          {#each values as value}
            <button
                class="w-full text-sm {py} px-2 text-left border-2 border-transparent hover:border-black rounded-base transition-colors text-black whitespace-nowrap block"
