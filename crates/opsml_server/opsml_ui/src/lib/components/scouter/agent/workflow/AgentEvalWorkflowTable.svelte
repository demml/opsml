<script lang="ts">
  ...
  import type { AgentEvalProfile, AgentEvalWorkflowPaginationResponse } from '../types';
  import type { RecordCursor } from '$lib/components/scouter/types';
  import { ArrowLeft, ArrowRight } from 'lucide-svelte';
  import AgentEvalWorkflowSideBar from './AgentEvalWorkflowSideBar.svelte';
  ...
  let {
    ...
    currentPage: AgentEvalWorkflowPaginationResponse;
    onPageChange: (cursor: RecordCursor, direction: string) => void;
    profile: AgentEvalProfile;
  }>();

  let workflows = $state<AgentEvalWorkflowResult[]>(currentPage.items || []);
  let selectedWorkflow = $state<AgentEvalWorkflowResult | null>(null);
  ...
  function selectWorkflowForDetail(workflow: AgentEvalWorkflowResult) {
...
<div class="pt-2 h-full flex flex-col min-h-0">
  ...
  {#if selectedWorkflow && isSelected}
    <AgentEvalWorkflowSideBar selectedWorkflow={selectedWorkflow} onClose={handleClosePanel} profile={profile} />
  {/if}
</div>