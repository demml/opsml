        ...
        <table class="w-full border-collapse text-left whitespace-nowrap">
            <thead class="bg-white text-xs uppercase text-slate-500 sticky top-0 z-10 shadow-sm">
                <tr>
                    ...
                    <th class="py-3 px-4 font-black text-left border-b-2 border-black bg-white">Metric</th>
                    <th class="py-3 px-4 font-black text-center border-b-2 border-black bg-white">Baseline</th>
                    <th class="py-3 px-4 font-black text-center border-b-2 border-black bg-white">Observed</th>
                    <th class="py-3 px-4 font-black text-center border-b-2 border-black bg-white">Threshold</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                {#each alertItems as alert}
                  {@const alertData = alert.alert.Custom}
                  <tr class="hover:bg-primary-50 transition-colors">
                    ...
                    <td class="py-3 px-4 text-xs font-bold text-slate-900">
                        <div class="truncate" title={alertData.metric_name}>
                          {alertData.metric_name}
                        </div>
                    </td>
                    <td class="py-3 px-4 text-center text-xs font-mono text-black">
                        {formatNumber(alertData.baseline_value)}
                    </td>
                    <td class="py-3 px-4 text-center text-xs font-mono text-black font-bold">
                        {formatNumber(alertData.observed_value)}
                    </td>
                    <td class="py-3 px-4 text-center text-xs font-mono text-error-700 font-bold">
                        {formatNumber(alertData.alert_threshold)}
                    </td>
                    <td class="py-3 px-4 text-center">
