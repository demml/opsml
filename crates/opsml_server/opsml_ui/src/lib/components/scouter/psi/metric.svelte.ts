...
import type { BinnedPsiFeatureMetrics, PsiDriftConfig } from "./types";

interface PsiStoreProps {
  config: PsiDriftConfig;
  ctx: DashboardContext; // Injected dependency
  initialMetrics: BinnedPsiFeatureMetrics;
}

export function createPsiStore({ config, ctx, initialMetrics }: PsiStoreProps) {
  // -- State --
  ...
  async function fetchAll(range: typeof ctx.timeRange, maxPoints: number) {
    isLoading = true;
    try {
      const newMetrics = await getPsiDriftMetrics(
        fetch,
        config.space,
        config.uid,
        range,
