<script lang="ts">
  ...
  import PsiConfigHeader from './PsiConfigHeader.svelte';
  ...
  import PsiAlertTable from './PsiAlertTable.svelte';
  import type { BinnedPsiFeatureMetrics, PsiDriftConfig, PsiDriftProfile } from './types';
  ...
  // Derived
  const profile = $derived(monitoringData.profiles[DriftType.Psi].profile.Psi as PsiDriftProfile);
  const config = $derived(profile.config as PsiDriftConfig);
  const metrics = $derived(monitoringData.selectedData.metrics as BinnedPsiFeatureMetrics);
    ...
    <div class="lg:col-span-3 flex flex-col gap-6">
      ...
      <div class="bg-white border-2 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-xl">
        <PsiConfigHeader
          ...
          profile={profile}
          profileUri={monitoringData.profiles[DriftType.Psi].profile_uri}
          uid={config.uid}
          ...
          {#if currentMetricData}
            {#key currentMetricName}
              <VizBody
                ...
                currentDriftType={DriftType.Psi}
