<script lang="ts">
  ...
  import type { GenAiToolActivity } from './types';
  import { fmtCompact, fmtMs, fmtPct } from './format';

  let { tools }: { tools: GenAiToolActivity[] } = $props();

  type SortCol = 'tool_name' | 'call_count' | 'avg_duration_ms' | 'error_rate';
  let sortCol = $state<SortCol>('call_count');
  ...
  const sorted = $derived([...tools].sort((a, b) => {
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-xs">
      <thead>
        <tr class="bg-surface-100 border-b-2 border-black">
          <th class="px-2 py-1 text-left text-xs font-black uppercase text-primary-700">
            <button onclick={() => toggle('tool_name')} class="hover:text-primary-950">
              Tool {sortCol === 'tool_name' ? (sortAsc ? '▲' : '▼') : ''}
            </button>
          </th>
          <th class="px-2 py-1 text-right text-xs font-black uppercase text-primary-700">
            <button onclick={() => toggle('call_count')} class="hover:text-primary-950">
              Calls {sortCol === 'call_count' ? (sortAsc ? '▲' : '▼') : ''}
            </button>
          </th>
          <th class="px-2 py-1 text-right text-xs font-black uppercase text-primary-700">
      ...
      <tbody>
        ...
        {:else}
          {#each sorted as row (row.tool_name)}
            <tr class="border-b border-black hover:bg-primary-100">
              <td class="px-2 py-1 text-xs font-mono text-primary-900 truncate max-w-[120px]">{row.tool_name ?? '—'}</td>
              <td class="px-2 py-1 text-xs font-mono text-primary-900 text-right">{fmtCompact(row.call_count)}</td>
