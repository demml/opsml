<script lang="ts">
  ...
  contentBase="card p-2 bg-surface-50 border-2 border-black shadow max-w-screen-xl w-[700px] max-h-[700px] overflow-auto"
  backdropClasses="backdrop-blur-sm"
>
  {#snippet trigger()}Show Features{/snippet}
  {#snippet content()}
  <div class="mx-auto rounded-2xl bg-surface-50 p-4 md:px-5 flex flex-col h-full">
    <!-- Header -->
      ...
      <div class="h-full overflow-y-auto overflow-x-auto">
        <div class="rounded-lg border-2 border-black overflow-hidden">
          <table class="w-full text-black text-sm bg-surface-100">
            <thead class="bg-primary-500 sticky top-0 z-10">
              <tr>
    ...
    <!-- Footer -->
    <footer class="flex justify-end gap-4 p-2 mt-6 flex-shrink-0">
