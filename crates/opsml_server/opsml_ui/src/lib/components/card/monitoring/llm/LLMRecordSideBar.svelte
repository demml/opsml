<script lang="ts">
  ...
  import type { LLMDriftServerRecord } from './llm';
  import LLMRecordContent from './LLMRecordContent.svelte';

  let {
    selectedRecord,
    onClose,
  }: {
    selectedRecord: LLMDriftServerRecord;
    onClose: () => void;
  } = $props();
...
>
  <LLMRecordContent record={selectedRecord} onClose={handleClose} showCloseButton={true} />
</div>