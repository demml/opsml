  ...
  <div class="flex flex-col gap-4">
    <div class="flex flex-row justify-between">
      <div class="flex flex-row">
        ...
        <header class="pl-2 text-primary-800 text-xl lg:text-2xl self-center font-bold">Parameters</header>
      </div>
      <button type="button" class="btn bg-primary-500 text-black shadow shadow-hover border-black border-2 self-center" onclick={() => setOpen(false)}>Close</button>
    </div>
      ...
      <table class="w-full text-black text-sm md:text-base bg-slate-100">
        ...
        <tbody>
          ...
          <tr class="border-t hover:bg-primary-300 py-2">
            <td class="p-2 text-center">{parameter.name}</td>
            <td class="p-2 text-center">{getParameterValue(parameter.value)}</td>
          </tr>
        {/each}
        </tbody>
      </table>
...
{:else}
  <div class="flex flex-row justify-between justify-between">
    ...
    <button type="button" class="btn bg-primary-500 text-black shadow shadow-hover border-black border-2 self-center" onclick={() =>setOpen(true)}>Open</button>
  </div>
{/if}
