use crate::core::scouter::{
    agent::get_scouter_agent_router, alerts::get_scouter_alert_router,
    drift::get_scouter_drift_router, genai::get_scouter_genai_router,
    health::get_scouter_health_router, observability::get_scouter_observability_router,
...
pub(crate) async fn get_scouter_router(prefix: &str) -> Result<Router<Arc<AppState>>> {
    ...
    let merged = Router::new()
        ...
        .merge(get_scouter_agent_router(prefix).await?)
        ...
        .merge(get_scouter_observability_router(prefix).await?);
