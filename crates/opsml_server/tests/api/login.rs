...
#[tokio::test]
async fn test_opsml_server_sso_programmatic_login() {
...
#[tokio::test]
async fn test_opsml_server_sso_ui_login() {
    let helper = TestHelper::new(Some("keycloak".to_string())).await;
    ...
    let expected_url = format!(
        "{}/realms/opsml/protocol/openid-connect/auth?client_id=opsml-client&response_type=code&scope=openid profile email&redirect_uri=http://localhost:8080/callback&state=test_state",
        mock_url
    );
