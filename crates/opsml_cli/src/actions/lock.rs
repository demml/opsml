...
use opsml_types::{
    RegistryType,
    contracts::{Card, CardRecord, ServiceType},
};
...
fn create_new_service_lock(
    ...
    let (service_card, _registered) = register_service_card(spec, registry, space, name)?;
...
pub fn lock_service_card(spec: &mut ServiceSpec) -> Result<LockArtifact, CliError> {
    ...
    let service_type = spec.service_type.clone();
    let service_version = spec
        .service
        .as_ref()
        .and_then(|s| s.version.as_ref())
        .cloned();
    let spec_cards: Option<Vec<Card>> = spec
        .service
        .as_ref()
        .and_then(|s| s.cards.as_ref().cloned());

    debug!("Locking service {}/{}", space, name);

    let registries = CardRegistries::new()?;
    let reg = match service_type {
        ServiceType::Api => &registries.service,
        ServiceType::Mcp => &registries.mcp,
        ServiceType::Agent => &registries.agent,
    };

    let existing_service = get_service_from_registry(reg, &space, &name, service_version.as_ref())?;

    match existing_service {
        None => {
            debug!("No existing service found, creating new service");
            create_new_service_lock(spec, reg, spec_cards.as_ref(), &space, &name)
        }
        Some(service) => {
            debug!("Existing service found, checking if refresh needed");
            handle_existing_service_lock(spec, reg, spec_cards.as_ref(), service)
        }
    }
}
...
pub fn lock_service(path: PathBuf) -> Result<(), CliError> {
    ...
    let lock_file = LockFile {
        artifact: vec![lock_service_card(&mut spec)?],
    };
