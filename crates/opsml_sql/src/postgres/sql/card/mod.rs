...
#[async_trait]
impl SkillLogicTrait for CardLogicPostgresClient {
...
#[async_trait]
impl ToolLogicTrait for CardLogicPostgresClient {
    ...
    async fn get_tool_card_by_version(
        ...
        let record = sqlx::query_as::<_, ToolCardRecord>(
            PostgresQueryHelper::get_tool_card_by_version_query(),
        )
        .bind(space)
        .bind(name)
        .bind(version)
    ...
    async fn list_tool_cards_by_space(
        ...
        let records = sqlx::query_as::<_, ToolCardRecord>(
            PostgresQueryHelper::get_list_tool_cards_by_space_query(),
        )
        .bind(space)
        .bind(tool_type)
        .fetch_all(&self.pool)
