...
#[derive(Debug, Clone)]
pub struct RoleLogicSqliteClient {
    pool: Pool<Sqlite>,
}

impl RoleLogicSqliteClient {
    pub fn new(pool: &Pool<Sqlite>) -> Self {
        Self { pool: pool.clone() }
    }
}

fn row_to_role(row: &sqlx::sqlite::SqliteRow) -> Result<Role, sqlx::Error> {
    use sqlx::Row;
    let permissions: Vec<String> =
        serde_json::from_value(row.try_get("permissions")?).unwrap_or_default();
    let is_system_int: i64 = row.try_get("is_system")?;
    Ok(Role {
        id: row.try_get("id")?,
...
#[async_trait]
impl RoleLogicTrait for RoleLogicSqliteClient {
    ...
    async fn seed_system_roles(&self) -> Result<(), SqlError> {
        ...
        for (name, description, permissions) in &system_roles {
            sqlx::query(
                "INSERT OR IGNORE INTO opsml_role (name, description, permissions, is_system) VALUES (?, ?, ?, 1)",
            )
            .bind(name)
            .bind(description)
            .bind(permissions)
