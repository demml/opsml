...
impl CardLogicTrait for CardLogicSqliteClient {
    ...
    async fn compare_hash(
        ...
        let mut query = format!(
            "SELECT space, name, version, uid, app_env, created_at FROM {table} WHERE content_hash = ?",
            table = table
        );
        ...
        match table {
            ...
            CardTable::Skill => match card {
                ServerCard::Skill(record) => {
                    let query = SqliteQueryHelper::get_skillcard_insert_query();
                    sqlx::query(query)
                        .bind(&record.uid)
                        .bind(&record.app_env)
                        .bind(&record.name)
            ...
            CardTable::SubAgent => match card {
                ServerCard::SubAgent(record) => {
                    let query = SqliteQueryHelper::get_subagent_card_insert_query();
                    sqlx::query(query)
                        .bind(&record.uid)
                        .bind(&record.app_env)
                        .bind(&record.name)
    ...
    async fn update_card(&self, table: &CardTable, card: &ServerCard) -> Result<(), SqlError> {
        match table {
            ...
            CardTable::Skill => match card {
                ServerCard::Skill(record) => {
                    let query = SqliteQueryHelper::get_skillcard_update_query();
                    sqlx::query(query)
                        .bind(&record.app_env)
                        .bind(&record.name)
                        .bind(&record.space)
            ...
            CardTable::SubAgent => match card {
                ServerCard::SubAgent(record) => {
                    let query = SqliteQueryHelper::get_subagent_card_update_query();
                    sqlx::query(query)
                        .bind(&record.app_env)
                        .bind(&record.name)
                        .bind(&record.space)
...
#[async_trait]
impl SubAgentLogicTrait for CardLogicSqliteClient {
    async fn get_subagent_card_by_name(
        ...
        let record = sqlx::query_as::<_, SubAgentCardRecord>(
            SqliteQueryHelper::get_subagent_card_by_name_query(),
        )
        .bind(space)
        .bind(name)
        .fetch_optional(&self.pool)
    ...
    async fn get_subagent_card_by_version(
        ...
        let record = sqlx::query_as::<_, SubAgentCardRecord>(
            SqliteQueryHelper::get_subagent_card_by_version_query(),
        )
        .bind(space)
        .bind(name)
        .bind(version)
    ...
    async fn list_subagent_cards_by_space(
        ...
        let records = sqlx::query_as::<_, SubAgentCardRecord>(
            SqliteQueryHelper::get_list_subagent_cards_by_space_query(),
        )
        .bind(space)
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn get_featured_subagents(
        ...
        let records = sqlx::query_as::<_, SubAgentCardRecord>(
            SqliteQueryHelper::get_featured_subagents_query(),
        )
        .bind(space)
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    async fn get_all_subagent_tags(&self, space: &str) -> Result<Vec<String>, SqlError> {
        let tags: Vec<String> =
            sqlx::query_scalar(SqliteQueryHelper::get_all_subagent_tags_query())
                .bind(space)
                .fetch_all(&self.pool)
                .await?;
        Ok(tags)
    ...
    async fn get_subagent_marketplace_stats(
        ...
        let row: (i64, i64, i64) =
            sqlx::query_as(SqliteQueryHelper::get_subagent_marketplace_stats_query())
                .bind(space)
                .fetch_one(&self.pool)
                .await?;
...
#[async_trait]
impl ToolLogicTrait for CardLogicSqliteClient {
    ...
    async fn get_tool_card_by_version(
        ...
        let record = sqlx::query_as::<_, ToolCardRecord>(
            SqliteQueryHelper::get_tool_card_by_version_query(),
        )
        .bind(space)
        .bind(name)
        .bind(version)
    ...
    async fn increment_tool_download_count(&self, uid: &str) -> Result<(), SqlError> {
        let result = sqlx::query(SqliteQueryHelper::get_increment_tool_download_count_query())
            .bind(uid)
            .execute(&self.pool)
            .await?;
        if result.rows_affected() == 0 {
    ...
    async fn list_tool_cards_by_space(
        ...
        let records = sqlx::query_as::<_, ToolCardRecord>(
            SqliteQueryHelper::get_list_tool_cards_by_space_query(),
        )
        .bind(space)
        .bind(tool_type)
        .bind(tool_type)
