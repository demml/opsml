    ...
    async fn get_users_paginated(
        ...
        let (users, total) = if let Some(search) = search {
            ...
            let users = sqlx::query_as::<_, User>(
                "SELECT id, created_at, active, username, password_hash, hashed_recovery_codes, permissions, group_permissions, favorite_spaces, role, refresh_token, email, updated_at, authentication_type FROM opsml_user WHERE username LIKE ? OR email LIKE ? ORDER BY username LIMIT ? OFFSET ?",
            )
            .bind(&pattern)
            .bind(&pattern)
            .bind(limit)
            ...
            let users = sqlx::query_as::<_, User>(
                "SELECT id, created_at, active, username, password_hash, hashed_recovery_codes, permissions, group_permissions, favorite_spaces, role, refresh_token, email, updated_at, authentication_type FROM opsml_user ORDER BY username LIMIT ? OFFSET ?",
            )
            .bind(limit)
            .bind(offset)
            .fetch_all(&self.pool)
