...
pub struct MySqlClient {
    ...
    pub card: CardLogicMySqlClient,
    pub exp: ExperimentLogicMySqlClient,
    pub artifact: ArtifactLogicMySqlClient,
...
impl MySqlClient {
    pub async fn new(settings: &DatabaseSettings) -> Result<Self, SqlError> {
        ...
        let client = Self {
            card: CardLogicMySqlClient::new(&pool),
            exp: ExperimentLogicMySqlClient::new(&pool),
            artifact: ArtifactLogicMySqlClient::new(&pool),
...
mod tests {
    ...
    async fn test_mysql_query_stats() {
        ...
        let stats = client
            .card
            .query_stats(&CardTable::Model, None, &vec![], &vec!["hello".to_string()])
            .await
            .unwrap();
        ...
        let stats = client
            .card
            .query_stats(&CardTable::Model, None, &vec![], &vec!["v3".to_string()])
            .await
            .unwrap();
